...
> 本文档分为两部分：
> - **基础篇（本文）**：模块概述、源码结构、HTTP 协议、头文件解析、数据流向、源码逐函数解析
> - **进阶篇**：关键技术点详解、epoll 详解、面试题精选 → [README_advanced.md](./README_advanced.md)
...
> **LT vs ET 模式**：详见 [进阶篇 11.5 ET/LT 模式详解](./README_advanced.md#115-etlt-模式详解)
> - LT（水平触发）：有数据就通知，可分多次读
> - ET（边缘触发）：只通知一次，必须循环读完
...
循环条件 `(m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK)` 看起来比较复杂，为什么要这样写？
...
│ Content-Length: 17\r\n                 │  ← 有 \r\n
│ \r\n                                   │  ← 空行有 \r\n
│ username=admin&pw=123                  │  ← 正文没有 \r\n！
└────────────────────────────────────────┘
```

**解析逻辑：**

| 阶段 | m_check_state | parse_line() 行为 | 循环进入条件 |
|------|---------------|-------------------|--------------|
| 解析请求行 | REQUESTLINE | 找到 \r\n，返回 LINE_OK | 条件2成立 |
| 解析头部 | HEADER | 找到 \r\n，返回 LINE_OK | 条件2成立 |
| 解析正文 | CONTENT | **找不到 \r\n，返回 LINE_OPEN** | **必须靠条件1** |

**条件1的作用**：当进入 CONTENT 状态时，`parse_line()` 因为找不到 `\r\n` 会返回 `LINE_OPEN`，导致条件2不成立。此时必须依靠条件1 `m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK` 才能进入循环体解析正文。

**line_status 的重置**：在解析完正文后，代码中 `line_status = LINE_OPEN;` 确保下一次循环时条件1不成立，从而退出循环。
...
> **mmap 零拷贝详解**：见 [进阶篇 7.3 mmap 内存映射](./README_advanced.md#73-mmap-内存映射零拷贝技术)
...
> **writev 分散写详解**：见 [进阶篇 7.4 writev 分散写](./README_advanced.md#74-writev-分散写高效发送响应)

#### ⚠️ writev 的重要注意事项

**`writev` 不会自动更新 `iovec` 结构体的指针和长度！**
...
当 `writev` 部分发送成功时（例如要发 1000 字节，只发了 600 字节），**`iovec` 的成员不会自动更新**，需要手动调整：

```
第一次 writev 调用：
┌──────────────────────────────────────────────────────────────────┐
│  iovec[0]: iov_base = 响应头起始, iov_len = 200                   │
│  iovec[1]: iov_base = 文件起始, iov_len = 800                     │
│  writev 返回: 600 (成功发送了 600 字节)                           │
└──────────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────────┐
│  问题：iovec 还是指向原来的位置！                                  │
│  iovec[0]: iov_base = 响应头起始, iov_len = 200  (没变)           │
│  iovec[1]: iov_base = 文件起始, iov_len = 800    (没变)           │
│                                                                   │
│  如果不调整，第二次 writev 会重复发送前面的数据！                  │
└──────────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────────┐
│  必须手动调整：                                                   │
│                                                                   │
│  情况1：响应头已发完 (bytes_have_send >= m_iv[0].iov_len)         │
│    m_iv[0].iov_len = 0;                                          │
│    m_iv[1].iov_base = 文件起始 + 已发送的字节数;                  │
│    m_iv[1].iov_len = 剩余字节数;                                  │
│                                                                   │
│  情况2：响应头还没发完                                            │
│    m_iv[0].iov_base = 响应头起始 + 已发送的字节数;                │
│    m_iv[0].iov_len = 响应头剩余字节数;                            │
└──────────────────────────────────────────────────────────────────┘
```
...
if (bytes_have_send >= m_iv[0].iov_len)
{
    // 响应头已全部发送完毕，调整到文件内容的剩余部分
    m_iv[0].iov_len = 0;                                          // 头部不再发送
    m_iv[1].iov_base = m_file_address + (bytes_have_send - m_write_idx);  // 文件偏移
    m_iv[1].iov_len = bytes_to_send;                              // 剩余长度
}
else
{
    // 响应头还没发完，继续发送响应头的剩余部分
    m_iv[0].iov_base = m_write_buf + bytes_have_send;             // 头部偏移
    m_iv[0].iov_len = m_iv[0].iov_len - bytes_have_send;          // 剩余长度
}
```
...
int setnonblocking(int fd)
{
    ...
    return old_option;                         // 返回旧标志
}
```

> **非阻塞 I/O 详解**：见 [进阶篇 7.5 非阻塞 I/O](./README_advanced.md#75-非阻塞-io)
...
> **epoll 详解**：见 [进阶篇 十一、epoll 详解](./README_advanced.md#十一epoll-详解)
...
**常见使用场景：**
- `modfd(..., EPOLLIN, ...)` - 继续等待请求数据
- `modfd(..., EPOLLOUT, ...)` - 准备发送响应
...
#### 6.20.0 前置知识：可变参数 va_list

`va_list` 是 C/C++ 中用于处理**可变参数函数**的类型。

**什么是可变参数函数？**

可变参数函数是指参数个数不确定的函数，比如 `printf()`：

```cpp
printf("%d", 10);              // 2个参数
printf("%d %s", 10, "hello");  // 3个参数
printf("%d %s %f", 10, "hi", 3.14);  // 4个参数
```

**相关宏定义：**

```cpp
#include <stdarg.h>  // C语言
#include <cstdarg>   // C++

va_list    // 类型：存储可变参数的列表
va_start   // 宏：初始化 va_list，开始访问可变参数
va_arg     // 宏：获取下一个参数
va_end     // 宏：结束访问，清理资源
```
...
int sum(int count, ...) {
    ...
    va_start(args, count);  // 2. 初始化，count 是最后一个固定参数

    int total = 0;
    for (int i = 0; i < count; i++) {
        total += va_arg(args, int);  // 3. 逐个获取参数，指定类型
    }
...
// 调用
sum(3, 1, 2, 3);       // 返回 6
sum(5, 1, 2, 3, 4, 5); // 返回 15
```

**流程图解：**

```
函数声明: void func(int fixed, ...)
                        │
                        ▼
              ┌─────────────────┐
              │  固定参数: fixed │
...
**注意事项：**
1. `va_start` 和 `va_end` 必须成对出现
2. `va_arg(args, int)` 中的类型必须与实际传入的类型一致
3. 需要通过固定参数（如 count 或 format）知道有多少可变参数
4. 编译时无法进行类型检查，需要程序员自己保证正确性
